...
import {
  DownloadStockClosingReportQuery,
  StockClosingReportFormat,
...
export class stockController {
  ...
  async createProductCategory(
    @Body() createProductCategoryRequestDto: CreateProductCategoryRequestDto,
  ) {
    return this.commandBus.execute(
      new CreateProductCategoryCommand(createProductCategoryRequestDto),
    );
  }
  ...
  async UpdateProductCategory(
    ...
    return this.commandBus.execute(
      new UpdateProductCategoryCommand(request, uuid),
    );
  }
  ...
  async createProductStock(
    @Body() createStockRequestDto: CreateStockRequestDto,
  ) {
    return this.commandBus.execute(
      new CreateStockCommand(createStockRequestDto),
    );
  }
  ...
  async getStocks(
    ...
    return this.queryBus.execute(
      new GetStocksQuery(size, offset, category, product, branch, workArea),
    );
  }
  ...
  @Post('validation')
  async validateStock(
    @Body() validateStockRequestDto: ValidateStockRequestDto,
  ) {
    return this.commandBus.execute(
      new ValidateStockCommand(validateStockRequestDto),
    );
  }

  @Get('report/download')
  async downloadStockHistoryReport(
    ...
    try {
      const excelBuffer = await this.queryBus.execute(
        new DownloadStockReportQuery(initialDate, endDate, branchId),
      );
  ...
  @Get('report/closing')
  async downloadStockClosingReport(
    @Query('date') date: Date,
    @Query('branchId') branchId: string,
    @Query('format') format: StockClosingReportFormat,
    @Res() res: Response,
  ) {
    try {
      const result = await this.queryBus.execute(
        new DownloadStockClosingReportQuery(date, branchId, format),
      );
      ...
      if (format === 'EXCEL') {
        ...
        res.setHeader(
          'Content-Disposition',
          'attachment; filename=cierre_vespertino.xlsx',
        );
  ...
  @Post('entrance')
  async createStockEntrance(
    @Body() createStockEntranceRequestDto: CreateStockEntranceRequestDto,
  ) {
    return this.commandBus.execute(
